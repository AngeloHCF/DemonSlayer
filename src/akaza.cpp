...
void Akaza::ChooseAttack(const Player& player, const Giyu* ally, const Shinobu* shinobu,
    ...
    if ((gActive || sActive || rActive || yActive || tActive || nActive) && GetRandomValue(0, 99) < 45) {
        float best = 1e9f;
        if (gActive) {
...
void Akaza::Update(float dt, Player& player, Giyu* ally, Shinobu* shinobu,
        ...
        case AkState::Shockwave: {
            ...
            for (auto& wv : waves) {
                ...
                if (wv.r > 20) {
                    ...
                    if (!wv.hitAlly && shinobu && shinobu->Active()) {
                        float d = Dist(shinobu->pos, wv.center);
                        if (fabsf(d - wv.r) < 28 && shinobu->pos.y > cfg::GROUND_Y - 120) {
                            wv.hitAlly = true;
                            float dir = shinobu->pos.x >= wv.center.x ? 1.0f : -1.0f;
                            shinobu->TakeDamage(18, dir * 520.0f, HitKind::BossAoe, fx);
                        }
                    }
                    if (!wv.hitAlly && rengoku && rengoku->Active()) {
                        float d = Dist(rengoku->pos, wv.center);
                        if (fabsf(d - wv.r) < 28 && rengoku->pos.y > cfg::GROUND_Y - 120) {
                            wv.hitAlly = true;
                            float dir = rengoku->pos.x >= wv.center.x ? 1.0f : -1.0f;
                    ...
                    if (!wv.hitAlly && sanemi && sanemi->Active()) {
                        float d = Dist(sanemi->pos, wv.center);
                        if (fabsf(d - wv.r) < 28 && sanemi->pos.y > cfg::GROUND_Y - 120) {
                            wv.hitAlly = true;
                            float dir = sanemi->pos.x >= wv.center.x ? 1.0f : -1.0f;
                            sanemi->TakeDamage(18, dir * 520.0f, HitKind::BossAoe, fx);
                        }
                    }
                }
            }
    ...
    for (auto& o : orbs) {
        ...
        if (o.alive && shinobu && shinobu->Active() &&
            CheckCollisionCircleRec(o.pos, 11, shinobu->Rect())) {
            shinobu->TakeDamage(orbDmg, (o.vel.x > 0 ? 1 : -1) * 300.0f,
                                HitKind::BossProjectile, fx);
            o.alive = false;
        }
        if (o.alive && rengoku && rengoku->Active() &&
            CheckCollisionCircleRec(o.pos, 11, rengoku->Rect())) {
            rengoku->TakeDamage(orbDmg, (o.vel.x > 0 ? 1 : -1) * 300.0f,
                                HitKind::BossProjectile, fx);
            o.alive = false;
        }
        ...
        if (o.alive && sanemi && sanemi->Active() &&
            CheckCollisionCircleRec(o.pos, 11, sanemi->Rect())) {
            sanemi->TakeDamage(orbDmg, (o.vel.x > 0 ? 1 : -1) * 300.0f,
                               HitKind::BossProjectile, fx);
            o.alive = false;
        }
...
void Akaza::TakeDamage(float dmg, float kbx, HitKind kind, Effects& fx) {
    ...
    if (kind == HitKind::Rengoku && vulnerable) mult *= 1.2f;
    ...
    if (kind == HitKind::Sanemi)  tcol = C(205, 245, 226);
...
int Akaza::NullifyOrbsInRect(Rectangle r) {
    int cut = 0;
    for (auto& o : orbs) {
        if (o.alive && CheckCollisionPointRec(o.pos, r)) {
            o.alive = false;
            cut++;
        }
    }
